...
//Todo create fetch alll products aput it in state and pass it ti=o the category page
  ...
  const fetchLatestProducts = async () => {
    const response = await axios.get("https://localhost:5000/products/latestproducts");
    setLatestProducts(response.data);
  };
  const fetchFavouriteProducts = async () => {
    const response = await axios.get("https://localhost:5000/products/favouriteproducts");
    setLatestProducts(response.data);
  };
